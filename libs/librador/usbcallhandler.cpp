...
void usbCallHandler::daq_for_channel(int channel, int numToGet, int interval_samples, daqUnitOptions unit_sel, FILE* iostream) {
    ...
    if(unit_sel==usbCallHandler::daqUnitOptions::Bits) {
        ...
        for(int i = 0; i < numToGet; i++) {
            ...
            fprintf(iostream, "%.0f ", buffer_for_daq->get_filtered_sample(ix, -1, 0, 0.0, false, true));
        }
    }
}
...
void usbCallHandler::drive_daq(int channel, int numToGet, int interval_samples, daqUnitOptions units_sel[2], const char * filepath) {
    ...
    FILE* iostream = open_file(filepath);
    buffer_read_write_mutex.lock(); 
...
FILE* usbCallHandler::open_file(const char * filepath) {
